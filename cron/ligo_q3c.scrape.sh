...
# +
#
# Name:        ligo_q3c.scrape.sh
# Description: scrape LIGO Q3C data from TNS
# Author:      Phil Daly (pndaly@email.arizona.edu)
# Date:        20190624
# Execute:     % bash ligo_q3c.scrape.sh --help
#
# -
...
usage () {
  write_blue   ""                                                                                                   2>&1
  write_blue   "Scrape LIGO Data from TNS (using Q3C indexing)"                                                     2>&1
  write_blue   ""                                                                                                   2>&1
  write_green  "Use:"                                                                                               2>&1
...
if [[ ${dry_run} -eq 1 ]]; then
  ...
  write_yellow "Dry-Run>> PYTHONPATH=${PARENT}:${PARENT}/src python3 ${PARENT}/src/utils/ligo_q3c_scrape.py --verbose --credentials=${credentials}"
...
# -
  ...
  write_yellow "Executing>> PYTHONPATH=${PARENT}:${PARENT}/src python3 ${PARENT}/src/utils/ligo_q3c_scrape.py --verbose --credentials=${credentials}"
  PYTHONPATH=${PARENT}:${PARENT}/src python3 ${PARENT}/src/utils/ligo_q3c_scrape.py --verbose --credentials=${credentials}
fi
