...
def sassy_bot_read(_radius=RADIUS, _begin=BEGIN_ISO, _end=END_ISO, _rb_min=RB_MIN, _rb_max=RB_MAX, _logger=None):
    ...
    _begin_iso = _begin if (re.match(DATE_RULE, _begin) is not None and iso_to_jd(_begin) is not float(math.nan)) else BEGIN_ISO
    ...
    try:
        db = DB(dbname=SASSY_DB_NAME, host=SASSY_DB_HOST, port=int(SASSY_DB_PORT), user=SASSY_DB_USER, passwd=SASSY_DB_PASS)
    ...
    # create new view
    _cmd_view = f'CREATE OR REPLACE VIEW sassy_bot ("objectId", jd, drb, rb, sid, candid, ra, dec) AS WITH e AS (SELECT "objectId", jd, rb, drb, id, candid, (CASE WHEN ST_X(ST_AsText(location)) < 0.0 THEN ST_X(ST_AsText(location))+360.0 ELSE ST_X(ST_AsText(location)) END), ST_Y(ST_AsText(location)) FROM alert WHERE ( (jd BETWEEN {_begin_jd} AND {_end_jd}) AND ((rb BETWEEN {_rb_min} AND {_rb_max}) OR (drb BETWEEN {_rb_min} AND {_rb_max})))) SELECT * FROM e;'
    if _logger:
    ...
    # select
    _cmd_select = f"WITH x AS (SELECT * FROM sassy_bot), y AS (SELECT x.*, (g.id, g.ra, g.dec, g.z, g.dist, q3c_dist(x.ra, x.dec, g.ra, g.dec)) FROM x, glade_q3c AS g WHERE q3c_join(x.ra, x.dec, g.ra, g.dec, {_radius:.5f})), z AS (SELECT * FROM y LEFT OUTER JOIN tns_q3c AS t ON q3c_join(y.ra, y.dec, t.ra, t.dec, {_radius:.5f})) SELECT * FROM z WHERE tns_id IS null;"
    if _logger:
...
if __name__ == '__main__':
    ...
    _p.add_argument(f'--radius', default=RADIUS, help=f"""Search radius, defaults to %(default)s''""")
    _p.add_argument(f'--rb-max', default=RB_MAX, help=f"""Deep-Learning real-bogus score maximum, defaults to %(default)s""")
    _p.add_argument(f'--rb-min', default=RB_MIN, help=f"""Deep-Learning real-bogus score minimum, defaults to %(default)s""")
