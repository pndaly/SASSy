...
def sassy_bot_read(_radius=RADIUS, _begin=BEGIN_ISO, _end=END_ISO, _rb_min=RB_MIN, _rb_max=RB_MAX, _logger=None):
    ...
    _begin_iso = _begin if (re.match(DATE_RULE, _begin) is not None
    ...
    # create new view
    _cmd_view = f'CREATE OR REPLACE VIEW sassy_bot ("objectId", jd, drb, rb, sid, candid, ra, dec) AS WITH e AS ' \
                f'(SELECT "objectId", jd, rb, drb, id, candid, (CASE WHEN ST_X(ST_AsText(location)) < 0.0 THEN ' \
                f'ST_X(ST_AsText(location))+360.0 ELSE ST_X(ST_AsText(location)) END), ST_Y(ST_AsText(location)) ' \
                f'FROM alert WHERE ( (jd BETWEEN {_begin_jd} AND {_end_jd}) AND ((rb BETWEEN {_rb_min} ' \
                f'AND {_rb_max}) OR (drb BETWEEN {_rb_min} AND {_rb_max})))) SELECT * FROM e;'
    if _logger:
    ...
    # select
    _res = None
    _cmd_select = f"WITH x AS (SELECT * FROM sassy_bot), y AS (SELECT x.*, (g.id, g.ra, g.dec, g.z, g.dist, " \
                  f"q3c_dist(x.ra, x.dec, g.ra, g.dec)) FROM x, glade_q3c AS g WHERE q3c_join(x.ra, x.dec, " \
                  f"g.ra, g.dec, {_radius:.5f})), z AS (SELECT * FROM y LEFT OUTER JOIN tns_q3c AS t ON " \
