...
# constant(s)
# -
TNS_CATALOG_FILE = os.path.abspath(os.path.expanduser('Tns.ans'))
...
# function: read_tns()
# -
def read_tns(_file='', force=False):
    ...
    if _all_lines is None:
        raise Exception(f'Failed to read {_file}, error={e}')
    ...
    for _e in _all_lines:
        ...
        for _k, _v in _dict.items():
            ...
            # {"41302": {"name": "AT 2019kst", "name_link": "https://wis-tns.weizmann.ac.il/object/2019kst", "reps": "1", 
            #            "reps_links": ["https://wis-tns.weizmann.ac.il/object/2019kst/discovery-cert"], "class": "", 
            ...
            if _tns_id > 0:
                ...
                # create record
                _tns = TnsRecord(tns_id=_tns_id, tns_name=_tns_name, tns_link=_tns_link,
                    ra=_ra, dec=_dec, redshift=_tns_redshift, discovery_date=_tns_discoverydate, discovery_mag=_tns_discoverymag,
                    discovery_instrument=_tns_discovering_instrument_name, filter_name=_tns_disc_filter_name, 
                    tns_class=_tns_class, host=_tns_hostname, host_z=_tns_host_redshift, source_group=_tns_source_group_name, 
                    alias=_tns_internal_name, certificate=_tns_certificate)

                # update database with results
                if _tns is not None and '0000-00-00' not in _tns_discoverydate:
                    try:
                        print(f"Inserting record = {_tns.serialized()}")
                        session.add(_tns)
                        session.commit()
...
if __name__ == '__main__':
    ...
    _p.add_argument(f'--force', default=False, action='store_true', help='if present, force update')
    args = _p.parse_args()

    # execute
    if args.file:
        read_tns(args.file, args.force)
    else:
