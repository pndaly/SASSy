...
# logging
# -
_log = UtilsLogger('TnsScrape').logger
...
class TnsTableParser(object):
    ...
    def __init__(self, url=DEFAULT_LOGIN_URL, credentials=DEFAULT_CREDENTIALS, number=DEFAULT_NUMBER,
        ...
        # verbose
        self.__log = UtilsLogger('TnsScrapeUserAgent').logger if self.__verbose else None
        if self.__verbose:
...
# -
# noinspection PyBroadException
def tns_scrape(login=DEFAULT_LOGIN_URL, credentials=DEFAULT_CREDENTIALS, number=DEFAULT_NUMBER,
    ...
    try:
        _log.info(f"Instantiating TnsTableParser('{login}', '{credentials}', '{number}', '{unit}', {verbose})")
        _ttp = TnsTableParser(login, credentials, number, unit, verbose)
        _log.info(f"Instantiated TnsTableParser('{login}', '{credentials}', '{number}', '{unit}', {verbose}) OK")
        _log.info(f"TnsTableParser.log = {_ttp.log}")
    except Exception as e:
...
if __name__ == '__main__':
    ...
    # execute
    if args:
        tns_scrape(args.login, args.credentials, int(args.number), args.unit, bool(args.verbose), bool(args.force))
    else:
