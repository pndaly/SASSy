...
class LigoTableParser(object):
    ...
    # +
    # method: scrape_ligo_events()
    # -
    def scrape_ligo_events(self):
        """ scrape web-site for ligo events """
    ...
    for _d in _data:
        for _ek, _ev in _d.items():
            ...
            try:
                ...
                _ligo = None
                if verbose:
                    _log.error(f"Failed to create LigoRecord for {_ev['name']}, error={e}")

            # update database with results
            if _ligo is not None:
                if dry_run:
                    if verbose:
                        _ligo.id = -1
                        _log.info(f"Dry-Run>> _ligo={_ligo.serialized()}")
                    if verbose:
