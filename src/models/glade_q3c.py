...
DB_VARCHAR_128 = 128
DB_VARCHAR_256 = 256
GLADE_Q3C_TXT_URL = 'http://glade_q3c.elte.hu/GLADE_2.3.txt'
...
# +
# function: glade_q3c_filters() alphabetically
# -
...
def glade_q3c_filters(query, request_args):
    ...
    if request_args.get('b_err__gte'):
        query = query.filter(GladeQ3cRecord.b_err >= float(request_args['b_err__gte']))
    # return records with b_err <= value (API: ?b_err__lte=12.5)
    if request_args.get('b_err__lte'):
        query = query.filter(GladeQ3cRecord.b_err <= float(request_args['b_err__lte']))
    ...
    if request_args.get('h_err__gte'):
        query = query.filter(GladeQ3cRecord.h_err >= float(request_args['h_err__gte']))
    # return records with h_err <= value (API: ?h_err__lte=0.5)
    if request_args.get('h_err__lte'):
        query = query.filter(GladeQ3cRecord.h_err <= float(request_args['h_err__lte']))
    ...
    if request_args.get('j_err__gte'):
        query = query.filter(GladeQ3cRecord.j_err >= float(request_args['j_err__gte']))
    # return records with j_err <= value (API: ?j_err__lte=0.5)
    if request_args.get('j_err__lte'):
        query = query.filter(GladeQ3cRecord.j_err <= float(request_args['j_err__lte']))
    ...
    if request_args.get('k_err__gte'):
        query = query.filter(GladeQ3cRecord.k_err >= float(request_args['k_err__gte']))
    # return records with k_err <= value (API: ?k_err__lte=0.5)
    if request_args.get('k_err__lte'):
        query = query.filter(GladeQ3cRecord.k_err <= float(request_args['k_err__lte']))
    ...
    if request_args.get('pgc__gte'):
        query = query.filter(GladeQ3cRecord.pgc >= int(request_args['pgc__gte']))
    # return records with pgc <= value (API: ?pgc__lte=20)
    if request_args.get('pgc__lte'):
        query = query.filter(GladeQ3cRecord.pgc <= int(request_args['pgc__lte']))
...
# +
# function: glade_q3c_get_text()
# -
def glade_q3c_get_text():
    return __text__


# +
# function: glade_q3c_cli_db()
# -
# noinspection PyBroadException
def glade_q3c_cli_db(iargs=None):
    ...
    if iargs.catalog:
        try:
            print(f'Receiving catalog from {GLADE_Q3C_TXT_URL}, this can take some time!')
            with urllib.request.urlopen(GLADE_Q3C_TXT_URL) as _catalog, open('glade_q3c.local.csv', 'w') as _wfd:
            ...
            print(f'Received catalog from {GLADE_Q3C_TXT_URL} OK')
        except Exception:
            pass
        return
    ...
    try:
        ...
        if iargs.verbose:
            print(f'query = {query}')
        query = glade_q3c_filters(query, request_args)
        if iargs.verbose:
            print(f'query = {query}')
    except Exception as e:
...
if __name__ == '__main__':
    ...
    _p = argparse.ArgumentParser(description=f'Query GLADE_Q3C database', formatter_class=argparse.RawTextHelpFormatter)
