...
# +
# class: GwgcRecord(), inherits from db.Model
# -
# noinspection PyUnresolvedReferences
class GwgcRecord(db.Model):
...
def gwgc_filters(query, request_args):
    ...
    if request_args.get('e_dist__gte'):
        query = query.filter(GwgcRecord.e_dist >= float(request_args['e_dist__gte']))
    ...
    if sort_order in SORT_ORDER:
        if sort_order.startswith(SORT_ORDER[0]):
            query = query.order_by(getattr(GwgcRecord, sort_value).asc())
        elif sort_order.startswith(SORT_ORDER[1]):
            query = query.order_by(getattr(GwgcRecord, sort_value).desc())
...
# +
# function: gwgc_cli_db()
# -
# noinspection PyBroadException
def gwgc_cli_db(iargs=None):
    ...
    try:
        ...
        query = session.query(GwgcRecord)
        if iargs.verbose:
            print(f'query = {query}')
        query = gwgc_filters(query, request_args)
        if iargs.verbose:
            print(f'query = {query}')
    except Exception as e:
    ...
    if isinstance(iargs.output, str) and iargs.output.strip() != '':
        try:
            with open(iargs.output, 'w') as _wf:
                ...
                for _e in GwgcRecord.serialize_list(query.all()):
